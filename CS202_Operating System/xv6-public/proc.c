...
int procnum = 0;  //to count the process number
...
//random function, the same as the one in usertests.c
...
//To get the total tickets of all processes
int
...
//To get the total strides of all processes
int
GetTotalStride(void)
{
    struct proc *p;
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
    if(p->parent == curproc){
        if(p->pgdir != curproc->pgdir) {
            ...
            //not to call the wakeup1 to wake up initproc
            p->parent = 0;
            p->state =ZOMBIE;
        }
    }
    ...
    //When one process exits, we should print all the processes' scheduled times to get the ratio.
...
scheduler(void)
{
  ...
  struct cpu *c = mycpu();
  c->proc = 0;
  int totaltickets;  //save the total tickets
  int randtickets;   //save the random number
  int totalstride;   //save the total strides
  for(;;){
    ...
    totaltickets = GetTotalTickets();  //get total tickets
    //if(totaltickets)
     //cprintf("%d\n",totaltickets);
    if(totaltickets < 0){
      ...
      //there are no tickets in all processes, run them by using round-robin or stride scheduling
      totalstride = GetTotalStride();  //get total strides
      //cprintf("123\n");
      ...
      else{   //run them by using stride scheduling
          int smallest = MAXSTRIDE;
          struct proc *schelit;  //target process
          int findit = 0;   //mark
    ...
    else{  //run them by using lottery scheduling
        //cprintf("%d\n",totaltickets);
      randtickets = rand();   //get the random number
      if(randtickets < 0) {
        randtickets = -1 * randtickets;
      }
      ...
      for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {  //choose a process to run
          if (p->state == RUNNABLE) {
              randtickets = randtickets - p->tickets;
          }
...
settickets(int t)    //initiate tickets to this process
{
  ...
  //t = rand();
  /*if(t == 0){
    curproc->tickets = MINTICKETS;
  }
  else if(t < 0){
    curproc->tickets = MINTICKETS;
  }
  else if(t > MAXTICKETS){
