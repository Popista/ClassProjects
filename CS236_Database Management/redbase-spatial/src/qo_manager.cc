  ...
  for(int i=0; i < nConds; i++){
    if(UseCondition(relsInJoin, i, newRel)){
      ...
      switch(conds[i].op){
        ...
        //case IS_OP : ApplyGTCond(attrStats, i, totalTuples); break;
        default: break;
      }
      int indexAttrTemp = -1;
      ...
      if(((conds[i].op == EQ_OP && (IsValidIndexCond(relsInJoin, i, newRel, indexAttrTemp) == 0)) || conds[i].op == IS_OP)
...
RC QO_Manager::CalcCondsForRel(map<int, attrStat> & attrStats, int relsInJoin, int relIdx,
  ...
  for(int i=0; i < nConds; i++){
    if(UseCondition(relsInJoin, i, relIdx)){
      ...
      if(conds[i].op == IS_OP && !conds[i].bRhsIsAttr && (attrs[attributeNum].indexNo != -1)){
        // update return values to reflect this index join
        useIdx = true;
        int unused;
        if((rc = CondToAttrIdx(i, attributeNum, unused)))
