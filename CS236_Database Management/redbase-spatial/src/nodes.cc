...
NODE *value_node(AttrType type, void *value)
{
    ...
    case STRING:
      ...
      break;
    case MBR:
      MBRS mmm = *(MBRS *)value;  //cast void type value to MBR type.
      cout << mmm.ctemp <<" \n";  //test
      n->u.VALUE.mval.ctemp = mmm.ctemp;
      s = mmm.ctemp;
      ...
      n->u.VALUE.mval.top_left_x = b[0];
      n->u.VALUE.mval.top_left_y = b[1];
      n->u.VALUE.mval.bottom_right_x = b[2];
      n->u.VALUE.mval.bottom_right_y = b[3];
      break;
    }
    return n;
}
